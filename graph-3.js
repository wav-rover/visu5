document.addEventListener("DOMContentLoaded", () => {
    ...
    function updateGraph(unit) {
        Promise.all([
            ...
            if (unit === "F") {
                data.forEach(d => d.temperature = convertToFahrenheit(d.temperature));
            }
            ...
            bars.on("mouseover", function(event, d) {
                tooltip.transition()
                    .duration(200)
                    .style("opacity", 1);
                tooltip.html(`Année: ${d.year}<br>Température moyenne: ${d.temperature.toFixed(1)}°${unit}`)
    ...
    const btnTemperature3 = document.getElementById("btnTemperature3");
    const unit = document.getElementById("unit");
    btnTemperature3.addEventListener("click", () => {
        currentUnit = currentUnit === "C" ? "F" : "C";
        updateGraph(currentUnit);
        unit.textContent = currentUnit === "C" ? "°C" : "°F";
    });
